...
pipeline {
    ...
    stages {
        ...
        stage('BDD-Test-k8s-1.13-and-1.14') {
            ...
            parallel {
                stage('BDD-Test-k8s-1.13') {
                    ...
                    steps {
                        sh 'cd ${TESTDIR}; ginkgo --focus="K8sDatapathConfig" -v --failFast=${FAILFAST} -- -cilium.provision=false -cilium.timeout=${GINKGO_TIMEOUT} -cilium.registry=$(./print-node-ip.sh)'
                    }
                    post {
                        always {
        ...
        stage('BDD-Test-k8s-1.15-and-1.16') {
            environment {
                CONTAINER_RUNTIME=setIfLabel("area/containerd", "containerd", "docker")
            }
            options {
